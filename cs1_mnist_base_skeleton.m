...
for iter=1:max_iter
      ...
      % Assigns the vectors to their closest centroid.
      
      [index, vec_distance] = assign_vector_to_centroid(train, centroids);
      
      % Updates the centroids based on the new assignments.
      
      new_centroids = update_Centroids(train, index);
      centroids = new_centroids;
      
      % Calculates the k-means "cost" at each iteration using the mean square
      % distance method.
      
      for v = 1:size(train(:,1))
          cost_iteration(iter) = cost_iteration(iter) + (vec_distance(v)).^2;
      end
      cost_iteration(iter) = cost_iteration(iter) / v;
...
index = zeros(size(data(:,1))).';
vec_distance = zeros(size(data(:,1))).';

% Assigns each of the test vectors to a centroid.

for v = 1:1500
    minDistance = [0, 0];
    for c = 1:size(centroids(:,1))
        distance = norm(data(v,1:784) - centroids(c,1:784));
        if (c == 1)
            minDistance = [c, distance];
        elseif distance < minDistance(2)
                minDistance = [c, distance];
        end   
    end
    index(v) = minDistance(1);
    vec_distance(v) = minDistance(2);
...
function new_centroids = update_Centroids(data, groups)
    
    % Uses splitapply to create the new centroids.
    
    new_centroids = splitapply(@mean, data(:, 1:784), groups.'); 
